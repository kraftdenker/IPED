...
public class WAContactsExtractorDesktop extends WAContactsExtractor {
    
    private static Logger logger = LoggerFactory.getLogger(WAContactsExtractorDesktop.class);

    private static final String SELECT_CONTACT_NAMES = "SELECT * FROM UserStatuses"; //$NON-NLS-1$

    private static final String SELECT_VERIFIED_NAMES = "SELECT jid, ContactName FROM UserStatuses where VerifiedLevel>0";
    
    public WAContactsExtractorDesktop(File database, WAContactsDirectory directory, boolean recoverDeletedRecords) {
    ...
    public void extractContactList() throws WAExtractorException {
        ...
        if (recoverDeletedRecords) {
            try {
                ...
                undelete.addTableToRecover("UserStatuses"); //$NON-NLS-1$
                undelete.addRecordValidator("UserStatuses", new WAAndroidContactValidator()); //$NON-NLS-1$
                undeletedContactsTable = undelete.undeleteData().get("UserStatuses"); //$NON-NLS-1$
            } catch (Exception e) {
        ...
        try (Connection conn = getConnection()) {
            try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(SELECT_CONTACT_NAMES)) {
                while (rs.next()) {
                    ...
                    c.setDisplayName(getString(rs, "ContactName")); //$NON-NLS-1$
                    c.setWaName(getString(rs, "UserName")); //$NON-NLS-1$
                    c.setNickName(getString(rs, "PushName")); //$NON-NLS-1$
                    c.setSortName(getString(rs, "FirstName")); //$NON-NLS-1$
                    c.setGivenName(getString(rs, "PushName")); //$NON-NLS-1$
                    c.setStatus(getString(rs, "Status")); //$NON-NLS-1$
                }
            }

            // Use verified names, if available, to replace undefined display names
            if (SQLite3DBParser.containsTable("UserStatuses", conn)) {
                try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(SELECT_VERIFIED_NAMES)) {
                    while (rs.next()) {
                        String verifiedName = getString(rs, "ContactName");
        ...
        if (undeletedContactsTable != null) {
            for (var row : undeletedContactsTable.getTableRows()) {
                ...
                if (! directory.hasContact(jid)) { // only recover contact if it does not exist already
                    ...
                    c.setDisplayName(nullToEmpty(row.getTextValue("ContactName"))); //$NON-NLS-1$
                    c.setWaName(nullToEmpty(row.getTextValue("UserName"))); //$NON-NLS-1$
                    c.setNickName(nullToEmpty(row.getTextValue("PushName"))); //$NON-NLS-1$
                    c.setSortName(nullToEmpty(row.getTextValue("FirstName"))); //$NON-NLS-1$
                    c.setGivenName(nullToEmpty(row.getTextValue("PushName"))); //$NON-NLS-1$
